    ...
    for j in range(len(semi_dataset)):
        if (not use_data[j]):
            ...
            if (confidence[cur_label] >= thresh and confidence[cur_label] >= configuration["model_params"]["semi_thresh"]):
                if (cur_label == emotion_label):
                    accurate_pseudo_labels += 1
                use_data[j] = True
                labels[j] = cur_label
                pseudo_labels_used += 1

    print('Pseudo-label accuracy: {:.2f}%'.format(100*accurate_pseudo_labels/pseudo_labels_used))
    print('Pseudo-labels generated: {:.2f}%'.format(100*pseudo_labels_used/len(semi_dataset)))
